...
public static class ResultLinq
{
    ...
    extension<T>(Result<T> result)
    {
        ...
        public Result<TResult> Select<TResult>(Func<T, TResult> map)
            => result.IsSuccess ? Result.Success(map(result.Value)) : Result<TResult>.Failure(result.Error);
        ...
        public Result<TResult> SelectMany<TSecond, TResult>(Func<T, Result<TSecond>> bind,
